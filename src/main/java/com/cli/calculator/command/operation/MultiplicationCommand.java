...
public class MultiplicationCommand implements Command, CommandValidator {
    @Override
    public Result execute(String[] command) throws Exception {
        if(isValidate(command)){
            double multi = Double.parseDouble(command[1]);
            for(int i = 2; i < command.length; i++){
                multi *= Double.parseDouble(command[i]);
            }
            return Result.builder().message("The Multiplication of numbers is: ").result(multi).build();
        }
    ...
    public boolean isValidate(String[] argument) throws Exception {
        if(!argument[0].equals("multi") || argument.length < 3){
            throw new Exception("Minimum two number required for multiplication operation!");
        }else {
            return true;
        }
    }
